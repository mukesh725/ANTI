    ...
    return (
      <main className="min-h-screen bg-[#FAF8F5]">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="w-8 h-8 border-4 border-[#597467]/20 border-t-[#597467] rounded-full animate-spin"></div>
        </div>
    ...
    return (
      <main className="min-h-screen bg-[#FAF8F5]">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          <h1 className="text-2xl font-serif text-[#597467] mb-4">Product Not Found</h1>
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F5]">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-12 md:py-24">
        ...
        <button 
          onClick={() => router.back()} 
          className="text-sm font-medium text-[#597467]/60 hover:text-[#597467] transition-colors flex items-center gap-2 mb-8 md:mb-16"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Shop
        </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
          ...
          >
            {product.badge && (
              <div className="absolute top-6 left-6 px-4 py-1.5 bg-[#597467] text-[#FAF8F5] text-[10px] font-bold uppercase tracking-[0.2em] rounded-full z-10">
                {product.badge}
              </div>
            )}
            <img 
          ...
          <div className="flex flex-col max-w-[500px]">
            ...
            <div className="mb-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#597467]/50">
                {product.category}
              </span>
            </div>
            
            <h1 className="text-4xl md:text-5xl font-serif text-[#597467] mb-4 leading-tight">
              {product.name}
            </h1>
            
            {product.tagline && (
              <p className="text-lg text-[#597467]/70 mb-8 font-medium">
                {product.tagline}
              </p>
            )}

            <div className="text-2xl font-serif text-[#597467] mb-8">
              ${Number(product.price).toFixed(2)}
              {product.weight && (
                <span className="text-sm font-sans text-[#597467]/50 ml-2">/ {product.weight}g</span>
              )}
            </div>

            <p className="text-base text-[#597467]/70 leading-relaxed mb-10">
              {product.description}
            </p>
            ...
            {!product.isComingSoon ? (
              <div className="flex items-center gap-4 mb-16">
                <div className="flex items-center border border-[#597467]/10 rounded-full bg-white h-14 w-32">
                  <button 
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="flex-1 flex items-center justify-center text-[#597467] hover:bg-[#597467]/5 rounded-l-full h-full transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center text-[#597467] hover:bg-[#597467]/5 rounded-r-full h-full transition-colors"
                  >
                ...
                <button 
                  ...
                  className={`flex-1 h-14 rounded-full text-sm font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                    added 
                      ...
                      : 'bg-[#597467] text-[#FAF8F5] hover:bg-[#E6AFA3] hover:text-[#597467]'
                  }`}
                >
              ...
              <div className="mb-16">
                <button 
                  disabled
                  className="w-full h-14 rounded-full bg-[#597467]/5 text-[#597467]/40 text-sm font-bold uppercase tracking-widest cursor-not-allowed"
                >
                  Coming Soon
                </button>
              </div>
            ...
            {/* ACCORDIONS */}
            <div className="border-t border-[#597467]/10">
              
              {ingredientsList.length > 0 && (
                <div className="border-b border-[#597467]/10">
                  <button 
                    onClick={() => setOpenAccordion(openAccordion === 'ingredients' ? null : 'ingredients')}
                    className="w-full py-6 flex items-center justify-between text-left group"
                  >
                    <span className="text-lg font-serif text-[#597467]">Ingredients</span>
                    <span className="text-[#597467]/40 group-hover:text-[#597467] transition-colors">
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    ...
                    <ul className="space-y-3">
                      {ingredientsList.map((ing, i) => (
                        <li key={i} className="text-sm text-[#597467]/70 flex items-start gap-3">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#E6AFA3] mt-1.5 flex-shrink-0" />
                          <span>{ing}</span>
                        </li>
                      ))}
                    </ul>
              ...
              {benefitsList.length > 0 && (
                <div className="border-b border-[#597467]/10">
                  <button 
                    ...
                    className="w-full py-6 flex items-center justify-between text-left group"
                  >
                    <span className="text-lg font-serif text-[#597467]">Benefits</span>
                    <span className="text-[#597467]/40 group-hover:text-[#597467] transition-colors">
