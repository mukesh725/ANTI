  ...
  return (
    <div className="min-h-screen bg-paper text-ink overflow-x-hidden selection:bg-theme selection:text-paper">
      ...
      <section className="relative h-[70vh] w-full flex items-center justify-start px-6 md:px-16 overflow-hidden pt-20">
        ...
        <div className="relative z-10 max-w-[800px] text-ink pt-12">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-theme/10 bg-theme/5 text-ink text-[9px] font-bold tracking-[0.25em] uppercase w-fit mb-8">
            <Leaf className="w-3 h-3 text-ink" /> Natural Nourishment
          </div>
          
          <h1 className="font-serif text-5xl md:text-7xl lg:text-[6rem] tracking-tight leading-[1.02] text-ink mb-8">
            AIRO <br/>
            <span className="italic font-light text-ink/80">Essentials.</span>
          </h1>
          
          <p className="font-sans text-sm md:text-base text-ink/70 max-w-lg leading-relaxed mb-12 tracking-wide">
      ...
      <section className="py-24 md:py-36 px-6 md:px-16 w-full max-w-[1500px] mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 className="font-serif text-4xl md:text-5xl tracking-tight">
            Curated <span className="italic font-light text-ink/80">Departments</span>
          </h2>
        </div>
            ...
            <div key={idx} className="flex flex-col group h-full">
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden mb-8 shadow-md">
                <ParallaxImage 
                  ...
                  speed={0.06}
                />
                <div className="absolute inset-0 bg-theme/5 mix-blend-multiply group-hover:bg-transparent transition-colors duration-500" />
              </div>
              <h3 className="font-serif text-2xl md:text-3xl text-ink mb-3">{cat.name}</h3>
              <p className="font-sans text-xs md:text-sm text-ink/70 leading-relaxed mb-6 flex-grow">
                {cat.desc}
              </p>
              <Link 
                href={cat.href} 
                className="mt-auto inline-flex items-center gap-2 text-[10px] tracking-widest uppercase font-bold text-ink hover:text-ink/60 silent-luxury-transition"
              >
