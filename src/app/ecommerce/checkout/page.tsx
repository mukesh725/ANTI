    ...
    return (
      <div className="min-h-screen bg-[#FFFFFF] text-[#1C1C1E] flex flex-col items-center justify-center p-6 text-center">
        <motion.div 
          ...
          className="bg-white p-12 rounded-3xl shadow-xl max-w-lg border border-[#1C1C1E]/5"
        >
          <div className="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-8">
            <Check className="w-10 h-10 text-white" />
          </div>
          <h1 className="font-serif text-4xl mb-4">Order Confirmed</h1>
          <p className="font-sans text-[#1C1C1E]/70 mb-8 leading-relaxed">
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-[#1C1C1E] pt-32 pb-16 px-6 md:px-16">
      <div className="max-w-[1200px] mx-auto">
        <h1 className="font-serif text-4xl mb-12 border-b border-[#1C1C1E]/10 pb-8">Checkout</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24">
            ...
            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required name="firstName" value={formData.firstName} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
                <div>
                  <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">Last Name</label>
                  <input required name="lastName" value={formData.lastName} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">Address</label>
                <input required name="address" value={formData.address} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">City</label>
                  <input required name="city" value={formData.city} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
                <div>
                  <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">State</label>
                  <input required name="state" value={formData.state} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">Zip Code</label>
                  <input required name="zip" value={formData.zip} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
                <div>
                  <label className="block text-[10px] uppercase tracking-widest font-bold mb-2">Phone</label>
                  <input required name="phone" value={formData.phone} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-[#1C1C1E]/10 rounded-lg" />
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-[#1C1C1E]/10 mt-8">
                ...
                <div className="bg-white p-6 border-2 border-[#1C1C1E] rounded-xl flex items-center gap-4">
                  <input type="radio" checked readOnly className="w-4 h-4 text-[#1C1C1E]" />
                  <div>
                    <h3 className="font-bold text-sm">Cash on Delivery</h3>
                    <p className="text-xs text-[#1C1C1E]/60">Pay with cash when your order is delivered.</p>
                  </div>
                </div>
              </div>
            </form>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-8 rounded-3xl border border-[#1C1C1E]/5 shadow-xl sticky top-32">
                ...
                {items.map(item => (
                  <div key={item.id} className="flex justify-between items-center text-sm">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-[#FFFFFF] rounded-md overflow-hidden relative flex-shrink-0">
              ...
              <button 
                ...
                type="submit"
                disabled={loading}
                className="w-full mt-8 bg-[#1C1C1E] text-[#FFFFFF] py-4 rounded-xl text-xs uppercase tracking-widest font-bold hover:bg-[#1C1C1E]/90 transition-colors disabled:opacity-50"
              >
