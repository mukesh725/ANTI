      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full max-w-md bg-paper shadow-2xl z-50 flex flex-col border-l border-ink/10"
          >
            {/* Header */}
            <div className="flex justify-between items-center p-8 border-b border-ink/10">
              <h2 className="font-serif text-2xl">Your Selection</h2>
              <button onClick={onClose} className="p-2 hover:bg-ink/5 rounded-full silent-luxury-transition">
                <X className="w-5 h-5" />
              </button>
            </div>
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <div className="flex items-center gap-2 bg-ink/5 rounded-full px-2 py-0.5">
                          ...
                          }} className="p-1 hover:text-ink/50"><Minus className="w-3 h-3" /></button>
                          <span className="text-xs font-medium w-4 text-center">{item.quantity}</span>
                          <button onClick={() => addItem(item)} className="p-1 hover:text-ink/50"><Plus className="w-3 h-3" /></button>
                        </div>
            ...
            {hasTurmeric && (
              <div className="bg-ink/5 m-8 p-6 rounded-sm border border-ink/10">
                ...
                <button className="text-xs uppercase tracking-widest font-medium border-b border-ink pb-1 hover:text-ink/70 silent-luxury-transition">
                  Book MinuteClinic
                </button>
              </div>
            )}
            ...
            <div className="p-8 bg-ink text-paper mt-auto">
              ...
              <Link href="/ecommerce/checkout" onClick={onClose} className="w-full py-4 bg-paper text-ink tracking-widest uppercase text-sm font-medium flex justify-center items-center gap-2 hover:bg-paper/90 silent-luxury-transition">
